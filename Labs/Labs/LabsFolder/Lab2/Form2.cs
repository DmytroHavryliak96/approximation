...
namespace Labs.LabsFolder.Lab2
{
    public partial class Form2 : Form
    {
                ...
                case "ln|cosx1| + tgx2 + ctgx3":
                    {
                        ...
                        for(int i = 0; i< dt.Rows.Count; i++)
                        {
                            desired[i][0] = Math.Log(Math.Abs(Math.Cos(input[i][0]))) + Math.Tan(input[i][1]) + 1/Math.Tan(input[i][2]);/*Math.Cos(input[i][0]) + Math.Tan(input[i][1]) + 1 / Math.Tan(input[i][2]);*/
                       ...
                       /* double average = sum / (desired.GetUpperBound(0) + 1);
                        ...
                        int example = 1;
                        int h = 0;
                        double[] value = new double[3] { 4, 6, 8};
                        ...
                        do
                        {
                            ...
                            {
                                double[] val; 
                                int temp = rdn.Next(i, input.GetUpperBound(0) + 1);
                                val = input[temp];
                                input[temp] = input[i];
                                input[i] = val;
                            }*/
        ...
        private void провестиТестToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            test.textBox4.Text = Convert.ToString(Math.Log(Math.Abs(Math.Cos(inputTest[0]))) + Math.Tan(inputTest[1]) + 1 / Math.Tan(inputTest[2]));
